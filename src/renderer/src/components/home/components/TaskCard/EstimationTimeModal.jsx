...
const EstimationTimeModal = ({ isOpen, onClose, task }) => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateTaskEstimationTime(task._id, totalSeconds, 1)).then((res) => {
      if (res.success) {
          ...
          <div className="flex justify-between w-full gap-2 h-full">
            ...
            <div className="w-1/4 border p-1.5 h-auto shadow-sm ">
              <div className="flex gap-4 mb-4 flex-col">
                <div className="text-base font-medium text-black text-left flex-grow border p-1 px-2 rounded bg-green-500/50 border-green-500">
                  PM Time : {task.pm_time ? formatTime(task.pm_time) : 'waiting for pm Time'}
                </div>
                <div className="text-base font-medium text-black text-left flex-grow border p-1 px-2 rounded bg-red-500/50 border-red-500">
                  CR Time :{' '}
                  {task.critical_time
                    ? formatTime(task.critical_time)
...
export default EstimationTimeModal
